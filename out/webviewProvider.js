...
class ChatPanel {
    static viewType = 'oneai.chatPanel';
    view;
    chatService;
    currentChatId = 'default';
    context;
    constructor(context) {
        this.chatService = new chatService_1.ChatService(context);
    ...
    async handleSendMessage(chatId, text) {
        ...
        if (connectionStatus.connector) {
            ...
            const sendPartialResponse = (partialText) => {
                if (this.view) {
                    this.view.webview.postMessage({
    ...
    async handleNewChat(chatId, chatName) {
    ...
    async handleConfirmDeleteChat(chatId) {
        if (chatId === 'default') {
            // Don't allow deletion of default chat
            if (this.view) {
                this.view.webview.postMessage({
        ...
        const result = await vscode.window.showInformationMessage(`Are you sure you want to delete the chat "${this.chatService.getChat(chatId)?.name || chatId}"?`, { modal: true }, 'Yes', 'No');
        if (result === 'Yes') {
    ...
    async handleSaveCurrentChatToFile(chatId) {
        try {
            ...
            vscode.window.showErrorMessage('Failed to save current chat: ' + error.message);
        }
    }
    async handleLoadCurrentChatFromFile() {
        try {
            // Show open dialog
            ...
            if (uri && uri[0]) {
                ...
                if (currentChat) {
                    currentChat.messages = loadedChat.messages.map((msgData) => {
            ...
            vscode.window.showErrorMessage('Failed to load current chat: ' + error.message);
        }
    }
    async handleSaveChatToFile() {
        try {
            // Get all chats data
            ...
            vscode.window.showErrorMessage('Failed to save chats: ' + error.message);
        }
    }
    async handleLoadChatFromFile() {
        try {
            // Show open dialog
            const uri = await vscode.window.showOpenDialog({
                filters: {
            ...
            vscode.window.showErrorMessage('Failed to load chats: ' + error.message);
        }
    }
    async handleDeleteAllChats() {
        // Show confirmation dialog
        const result = await vscode.window.showInformationMessage('Are you sure you want to delete all chats? This cannot be undone.', { modal: true }, 'Yes', 'No');
        if (result === 'Yes') {
